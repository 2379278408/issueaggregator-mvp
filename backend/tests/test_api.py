...
class ApiTestCase(unittest.TestCase):
    ...
    def make_request_stub(
        self, *, headers: dict[str, str] | None = None, client_host: str | None = None, path: str = '/api/feedback'
    ):
    ...
    def test_admin_token_dependency_logs_failed_attempt(self) -> None:
        ...
        try:
            with self.assertLogs('app.routers.feedback', level='WARNING') as captured, self.assertRaises(HTTPException):
                self.require_admin_token(request, 'wrong-token')
        finally:
            self.feedback_router_module.settings = original_settings
