...
def admin_logout(request: Request, response: Response) -> dict[str, object]:
    ...
    if session_token:
        ...
        if session and not session.revoked_at:
            ...
            _write_audit_event(
                'admin_session_revoked', client_ip, request.url.path, action='logout', resource_id=session.id
            )
...
def require_admin_token(
    ...
    if settings.admin_api_token and x_admin_token and compare_digest(x_admin_token, settings.admin_api_token):
        return
